...
class PendingSend(Base):
    ...
    # True when the user picked a future send time ("send later"), as opposed to
    # the transient 8s undo window. Only these rows appear in the Scheduled list.
    scheduled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
