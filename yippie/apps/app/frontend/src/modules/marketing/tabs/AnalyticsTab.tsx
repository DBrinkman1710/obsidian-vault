  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-xs">
        <span className="font-semibold uppercase text-slate-700">{t('mkt_variant')} {v.variant}</span>
        <span className="text-slate-500">
          {openRate}% {t('mkt_open')} · {v.sent} {t('mkt_stat_sent').toLowerCase()}
        </span>
      </div>
...
export function AnalyticsTab({ campaign }: { campaign: Campaign }) {
  ...
  if (isLoading || !data) {
    return <div className="p-6 text-sm text-slate-400">{t('mkt_loading_analytics')}</div>
  }
    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <Stat label={t('mkt_stat_sent')} value={data.sent} />
          <Stat label={t('mkt_stat_opened')} value={`${data.open_rate}%`} sub={`${data.opened} ${t('mkt_stat_contacts')}`} />
          <Stat label={t('mkt_stat_clicked')} value={`${data.click_rate}%`} sub={`${data.clicked} ${t('mkt_stat_contacts')}`} />
        ...
        {buttonData.length > 0 && (
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="border-b border-slate-100 px-4 py-3">
              <h3 className="text-sm font-semibold text-slate-900">{t('mkt_button_clicks')}</h3>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                  <th className="px-4 py-2.5">{t('mkt_col_button')}</th>
                  <th className="px-4 py-2.5">{t('mkt_col_clicks')}</th>
                  <th className="px-4 py-2.5">{t('mkt_col_action')}</th>
                  <th className="px-4 py-2.5">{t('mkt_col_outcome')}</th>
                </tr>
              </thead>
              <tbody>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                <th className="px-4 py-2.5">{t('mkt_col_recipient')}</th>
                <th className="px-4 py-2.5">{t('mkt_col_status')}</th>
                <th className="px-4 py-2.5">{t('mkt_col_variant')}</th>
                <th className="px-4 py-2.5">{t('mkt_col_reply')}</th>
              </tr>
            </thead>
            <tbody>
