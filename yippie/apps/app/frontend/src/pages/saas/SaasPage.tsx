...
import { Copy, Check, BarChart3, AlertTriangle, Users, Zap } from 'lucide-react'
...
export default function SaasPage() {
  ...
  const snippetScript = tokenData
    ? `<script src="https://getyippie.com/saas.js" data-token="${tokenData.tracking_token}" async></script>`
    : ''

  const usageExample = `// After user login:
yippie.identify('user-123', { email: 'jan@acme.nl', name: 'Jan', plan: 'starter' });

// When a feature is used:
yippie.track('feature_used', { feature: 'csv_export' });

// When an onboarding step completes:
  ...
  return (
    <div className="max-w-4xl space-y-8">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-slate-700">Installation</h2>
          <p className="text-xs text-slate-500">
            Add this tag to your SaaS product's HTML, then call <code className="bg-slate-100 px-1 rounded">yippie.identify()</code> after login.
          </p>
          ...
          <pre className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-600 font-mono overflow-x-auto">
            {usageExample}
          </pre>
        </div>
        ...
        {summary && summary.total_contacts_tracked === 0 && (
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-xs mt-1">Install the snippet in your SaaS product and call identify() after login.</p>
          </div>
        )}
    </div>
  )
