...
- **[SENT2] Sent mail content blank when opened** — `Sonnet` — clicking a sent mail in the Sent tab opens the thread/detail view but the email body does not render (blank content area). Fix so the outbound email body and any reply chain display correctly in the detail view. See Tier 3.
- **[CAM-SIMP] Campaign button actions: simplify to pipeline stage only** — `Sonnet` — the button-actions panel currently offers 5 types (apply label, pipeline stage, open website, send mail, call phone); user wants only **pipeline stage**. Remove apply-label, open-website, send-mail, call-phone from `TemplatesPage` button-actions UI and from `build_direct_action_href()` / token dispatch. Simplify `CampaignButton.action_type` to `pipeline_stage` only. See Tier 3.
- **[BK2] Calendar: + New Booking button** — `Sonnet` — no way to create a booking directly on the Calendar page; booking settings section sits below the calendar grid and forces scrolling. Add a **+ New Booking** button in the Calendar header (same row as + New Event) that opens a modal where the agent fills in contact + notes and sends the booking link — reuses `POST /booking/send`. Remove or collapse the booking settings section from below the calendar so the page fits without scrolling; settings can live behind a gear icon or be moved elsewhere. See Tier 3.
- **[BK3] Booking: propose specific dates** — `Opus` — when sending a booking link the agent currently can't propose concrete time slots. Add a "Propose dates" mode to `SendBookingModal`: agent picks 2–4 specific date+time options; `booking_tokens` stores them as a `proposed_slots` JSON array; the public `/book/:token` page renders only those slots (accept one, decline). Complements the existing open slot-picker (contact picks any free slot). See Tier 2.
