...
### UX polish
- **[BR1] Branding colour — "Reset to default" in client edit + editable in Settings** — `Sonnet` — two parts: (1) In the SuperAdmin Clients → Edit → Branding tab, add a "Reset to default" button next to the `primary_color` colour picker that sets the value back to `#5BA4F5` (the platform default blue); (2) In the tenant Settings page (accessible to tenant admins), expose a `primary_color` picker so admins can change their own branding colour without needing a superadmin. Backend: reuse existing `PATCH /tenant/config` or add a new `PATCH /settings/branding` endpoint gated to `AdminUser`. See Tier 3.
