...
**[FLOW8] Built in automations surfaced on /flows** — `Opus` — *after [FLOW7].* Clients should see what Yippie already automates for them — and where the flow engine can express it, own and edit it as a real flow.
- **Inventory (tenant facing, verified against code 2026-07-08):** tickets SLA escalation every 5 min + auto close stale hourly (`tickets/automation/sla_escalation.py`); inbox AI scan inbound → draft ticket (`inbox/ai_scanner.py`); Yip SLA nudge every 10 min + morning briefing (`jarvis/scheduler.py`); contract renewal reminders 6h (`contracts/scheduler.py`); campaign scheduled send / A/B winner pick / drip campaigns / monthly engagement decay (`marketing/scheduler.py`); SaaS health compute hourly + Monday at risk digest (`saas/scheduler.py`); invoice overdue check daily (in `sla_escalation.py`); livechat auto claim on first agent reply; ERP order webhook → contact sync (shipments). **Never show:** platform internal jobs (demo nudge/expiry, subscription expiry, contact retention purge, external calendar sync, thread cleanup).
- **Phase A — visibility:** static catalogue `flows/builtins.py` (key, name, one line description, module, cadence, optional settings deep link), served in `/flows/meta`, filtered by `enabled_modules` exactly like `TRIGGER_META`. FlowsPage gets a "Built in automations" section (styled like the recipes strip) of read only cards — the "what's possible" picture.
- **Phase B — convert the expressible ones into default flows:** some built ins are already expressible in the engine (Yip SLA nudge ≈ `ticket_sla_due_soon` + `notify_user`; overdue escalation ≈ `ticket_sla_due_soon` + `update_ticket` priority). Install those as real, enabled, editable flows (`is_default` marker column or config key for the badge) at tenant seed + a one time backfill for existing tenants. **Critical:** the legacy hardcoded job must stop firing for tenants that have the default flow — otherwise double notifications. Simplest: ship the backfill and retire the legacy job in the same deploy. Non expressible ones (auto close scan, AI inbox scan, health compute — they need per entity scans or AI calls the engine doesn't have) stay Phase A cards.
...
- **[FLOW8] Flows: built in automations surfaced on /flows** — `Opus` — *after [FLOW7].* Phase A: read only "Built in automations" cards from a `flows/builtins.py` catalogue filtered by enabled modules. Phase B: convert expressible ones (SLA nudge, overdue escalation) into default installed editable flows + retire the legacy jobs in the same deploy. Spec above.
