...
- **[BK-HOURS-BUG] Booking: work hours not enforced — slots outside 9:00–17:00 visible** — `Haiku` — Calendar settings show 9:00–17:00 but the public booking page shows slots from ~12:00–19:00 or similar wrong range. Root cause likely in `booking/service.get_available_slots()`: check whether `work_start`/`work_end` comparison uses the correct timezone, or whether `use_weekly_slots` mode is accidentally overriding work hours without honouring them. Files: `booking/service.py`, `BookingPage.tsx`. See Tier 3.
...
**Features — app:**
- **[TPL-CONSISTENT] Template editor: consistent layout — presets left, editor right; marketing templates fix; module gating** — `Sonnet` — (1) `TemplatesPage.tsx` shows template list left + GrapesJS editor right — keep this two-column layout; when editing a template from anywhere (InboxQueue, DraftReview) it must always open in the same popup, shifted to the right so the preset list is visible on the left. (2) Marketing templates (`GET /marketing/templates` or `campaign_templates`) do not appear in the `TemplatePicker` in `InboxQueue.tsx` and `DraftReview.tsx` — templates and campaign templates are the same concept stored in two places; ensure TemplatePicker fetches from both (or unify). (3) Hide the Templates button in InboxQueue compose and DraftReview when the tenant does not have the marketing module enabled — gate via `enabledModules`. Files: `TemplatesPage.tsx`, `TemplatePicker.tsx`, `InboxQueue.tsx`, `DraftReview.tsx`. See Tier 2.
- **[BK-MULTISLOT] Booking: multiple distinct time slots per day in weekly schedule** — `Sonnet` — The weekly schedule grid (`WeeklyGrid` in `BookingSettingsModal.tsx`) needs to support multiple distinct time slots per day — e.g. 09:00 × 4 capacity, 11:30 × 4 capacity, 15:00 × 4 capacity all on the same day. The backend `weekly_slots JSONB` already stores an array per day; verify `get_available_slots()` handles multiple entries per day correctly. The UI should let admins click `+ Add time` to add more rows per day, each with its own `HH:MM` + capacity picker and a `✕` to remove. Files: `BookingSettingsModal.tsx`, `booking/service.py`. See Tier 2.
...
**Pricing:**
- **[PLAN-STARTER] Starter plan: max 3 users or price update** — `Haiku` — Decision: keep starter at €19/month as the low-friction entry hook (module add-ons drive revenue). Optionally cap starter at 3 users. Update `PLAN_LIMITS` in `plans.py` and the getyippie.com pricing page card to reflect. If keeping €19 with no cap change, just close. Files: `plans.py`, `apps/web/src/app/pricing/page.tsx`. See Tier 3.
...
- **[TPL-CONSISTENT] Template editor: consistent layout + marketing templates + module gating** — `Sonnet` — *not built.* (1) Editing a template always opens a popup shifted right (preset list visible on left). (2) Marketing campaign templates not appearing in `TemplatePicker` — fetch from both `GET /templates` and `GET /marketing/templates` (or unify). (3) Hide Templates button in InboxQueue/DraftReview when marketing module is disabled. Files: `TemplatesPage.tsx`, `TemplatePicker.tsx`, `InboxQueue.tsx`, `DraftReview.tsx`.
- **[BK-MULTISLOT] Booking: multiple distinct time slots per day in weekly schedule** — `Sonnet` — *not built.* `WeeklyGrid` in `BookingSettingsModal.tsx` currently supports one time slot per day. Allow `+ Add time` for multiple `{time, capacity}` rows per day-of-week, each removable with `✕`. Backend `get_available_slots()` already stores an array per day — verify it handles multiple entries correctly. Files: `BookingSettingsModal.tsx`, `booking/service.py`.
...
- **[BK-HOURS-BUG] Booking: work hours not enforced — slots outside 9:00–17:00** — `Haiku` — *not built.* Booking page shows slots well outside the configured work hours (e.g. 12:00–19:00 instead of 9:00–17:00). Check timezone handling and `use_weekly_slots` mode override in `booking/service.get_available_slots()`. Files: `booking/service.py`.
...
- **[PLAN-STARTER] Starter plan: max 3 users or keep at €19** — `Haiku` — *not built.* Leaning toward €19/month as low-friction hook (modules drive revenue), optionally cap starter at 3 users. Update `plans.py` `PLAN_LIMITS` + getyippie.com pricing card.
---
