...
**Features — app:**
- **[BK8] Booking: per-send post-booking pipeline stage override** — ✅ DONE — `stage_id_override` nullable UUID FK on `booking_tokens` (migration `bk8_booking_token_stage_override`); `BookingTokenCreate` + schema updated; `confirm_booking()` uses override over global setting; `SendBookingModal` "Move to stage" dropdown.
...
- ~~**[MODULE-INC] Correct included vs paid modules**~~ ✅ **DONE (session b-20260622-module-pricing)** — `CORE_FEATURES` in `plans.py` reduced to inbox/contacts/activity; tickets/billing moved to `ADVANCED_FEATURES`; `MODULE_PRICES` extended with chat/marketing/departments/billing; `web/src/lib/config.ts` updated to match; pricing page `CORE_FEATURES` set to ["Inbox","Contacts","Activity"].
- ~~**[MODULE-RENAME] Rename emailtracking → "Marketing module"; remove Templates as standalone module**~~ ✅ **DONE (session b-20260622-module-pricing)** — `emailtracking` was already absent from `ALL_MODULES` and `Sidebar.tsx` `MODULE_MAP`; standalone Templates entry never existed in sidebar; emailtracking webhooks remain mounted in `main.py` (Resend delivery events); no sidebar or route changes needed beyond confirming the state.
- ~~**[MODULE-CONSIST] Commercial site: pricing page lists all modules consistently**~~ ✅ **DONE (session b-20260622-module-pricing)** — pricing page `addOns` now covers all 8 paid modules (Tickets, AI, Calendar + Booking, Kanban, Live Chat, Marketing, Departments, Billing); questionnaire `featureOptions` updated to match; modules page updated: emailtracking entry replaced with Marketing, added Departments + Billing entries, hero updated to "Twelve modules".
...
- **[BK8] Booking: per-send post-booking pipeline stage override** — ✅ DONE — `stage_id_override` on `booking_tokens` (migration `bk8_booking_token_stage_override`); `SendBookingModal` "Move to stage" dropdown; `confirm_booking()` uses override over global setting.
...
- ~~**[MODULE-INC] Correct included vs paid module split**~~ ✅ **DONE (session b-20260622-module-pricing)** — see Tier 2 body above.
- ~~**[MODULE-RENAME] Rename emailtracking → Marketing; remove Templates as standalone**~~ ✅ **DONE (session b-20260622-module-pricing)** — see Tier 2 body above.
- ~~**[MODULE-CONSIST] Commercial site: pricing page covers all modules**~~ ✅ **DONE (session b-20260622-module-pricing)** — see Tier 2 body above.
