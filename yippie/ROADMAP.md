...
- [ ] [LIVECHAT-EVOLUTION-LICENSE] Evolution API attribution notice in settings (legal requirement)
...
- **[GOLIVE-CHECKLIST] Write the go-live checklist** — `Sonnet` — before any go-live work starts, produce a single checked-off list of everything that must be true on 2026-06-28: Railway env vars set, DNS records verified, INBOUND_EMAIL live, Resend domains verified, superadmin account created on prod, seed data run, smoke-test script, Cloudflare proxy status, any feature-flag or module toggles that need flipping. Output as a section in this ROADMAP (or a standalone `GOLIVE.md`). Do this first so nothing is missed on the day. See Tier 3.
...
- **[CAM1] Email Campaign Creator module** — `Fable` — full campaign lifecycle built on top of [GRAPES1]: campaigns + campaign_templates + campaign_analytics DB tables; GrapesJS campaign designer canvas; Resend batch dispatch with open-tracking pixel; reply tracking via inbound webhook + AI classifier; Activity timeline campaign filter + analytics card (Sent / Opened% / Reply% / sentiment bar). Deferred until after June 28 — depends on [GRAPES1] merge. See Tier 1.
...
**Deploy reminder:** staging deploys from `devsandbox` — ship with `git push origin devsandbox`. Both Sandbox-env services (`Dev Sandbox` + `Sandbox`) rebuild from this branch automatically. `migrations/env.py` takes a Postgres advisory lock so the two shared-DB containers can't race DDL. Marketing site: `git push origin commercial`.
...
- **[CAM1] Email Campaign Creator module** — `Fable` — *not built.* Full campaign lifecycle: build → launch → track → analyse. **Depends on [GRAPES1] being merged first.**
  - **DB** — migrations (tenant-scoped): `campaigns` (id, tenant_id, name, subject, status enum draft/sending/completed, created_at, updated_at); `campaign_templates` (id, campaign_id, raw_html, raw_css); `campaign_analytics` (id, tenant_id, campaign_id, recipient_email, status enum sent/opened/replied, tracking_token UUID unique, reply_classification nullable, updated_at); nullable `campaign_id` FK added to the activity log table.
  - **Frontend: Campaign Designer** — "Start New Campaign" button on TemplatesPage → `/campaigns/new`; Campaign Name + Subject inputs; embedded GrapesJS canvas (newsletter preset); "Launch Campaign" extracts `editor.getHtml()` + `editor.getCss()`, saves to `campaign_templates`, triggers launch endpoint.
  - **Backend pipeline** — `POST /campaigns/launch`: chunks recipient list from contacts DB (or segment filter); injects 1×1 tracking pixel `<img src="/track/open/:token">` into HTML before send; pushes to Resend via existing `flush_pending_sends` queue; creates one `campaign_analytics` row per recipient.
  - **Open tracking** — `GET /track/open/:token` (public): marks analytics row `opened`, returns 1×1 transparent GIF.
  - **Reply tracking** — inbound Resend webhook matches `campaign_id` from `campaign_analytics` by sender email; sets status `replied`; runs AI reply classifier → writes `reply_classification` (Interested / Opt-out / Out of office / Other).
  - **Activity timeline** — "Filter by Campaign" dropdown on ActivityFeed; selecting a campaign isolates its events. Analytics card at top: Total Sent, Opened %, Reply %; sentiment bar showing reply classification breakdown.
  - Module toggle `"campaigns"` in `ALL_MODULES`; sidebar Campaigns link.
...
- **[BIZ-Q1] Business questionnaire → module recommendation** — `Fable` — *not built.* Extend the `/request-demo` page with a short qualifying questionnaire (team size, industry, current tooling, top pain points) that maps answers to a recommended set of Yippie modules. Show the recommendation inline before the user submits. The `POST /public/request-demo` backend records the answers in the Ticket body so the root-tenant agent has full context when following up. Coordinates with and may supersede [CUSTOM1]. See Tier 1.
...
### Live Chat
- **[LIVECHAT-STATUS-MODEL] Full chat status model + assignment** — `Opus` — Status lifecycle: **open → assigned → solved → ticket** (ticket escalated via [LIVECHAT-TICKET]). **Auto-assign on open**: when an agent clicks a session it is automatically assigned to them (status → `assigned`); session disappears from all other agents' lists and they cannot reply until the assignee changes status back to `open` or re-assigns. **Re-assign** button in session header (agent picker dropdown). **Filter tabs** in sidebar: Mine / Open / All. **Browser notifications** only fire for the assigned agent (unassigned → all agents). **Solved**: session is hidden from the active list after N hours (configurable per tenant in Settings, default 72h) but **messages and session data are retained permanently** for AI context history — auto-hide is a display filter, not a deletion. **Settings**: add "Hide solved chats after (hours)" input to a LiveChat section in `/settings`. DB: add `status VARCHAR(20)` (`open`|`assigned`|`solved`|`ticket`) + `assigned_to UUID FK (users)` + `solved_at TIMESTAMPTZ` + migration. List endpoint filters by `status + assigned_to` based on tab. Files: `ChatSession` model + migration, `ChatPage.tsx`, `chat/router.py`, `LabelsPage.tsx` (settings).
...
### Kanban / Pipeline
- **[DEMO-WF1] Demo-request → Kanban "Demo requested" stage** — `Opus` — *not built.* When `POST /public/request-demo` fires, automatically place the created contact into a Kanban stage named **"Demo requested"** in the root tenant (create the stage at `display_order=0` if it doesn't exist, via the existing `pipeline_stages` CRUD). Currently Phase 12 creates a Contact + Ticket but does not call `_assign_stage`. This closes the full loop: *demo requested → contact created → placed in Kanban*. Prerequisite: sandbox → live promotion complete.
- **[KANBAN-STAGES] Customisable Kanban stages per tenant** — `Opus` — *not built.* Each tenant should be able to configure their own stages (add / rename / reorder / delete) via a Settings panel. `pipeline_stages` already has `tenant_id`, so check whether the backend CRUD is already in place (`GET/POST/PATCH/DELETE /pipeline/stages`) — if so, only the Settings UI is missing. Default stages provisioned at tenant creation. Depends on [KAN-RN] (rename) being done first.

### Pipeline × Activity
- **[P1] Stage filter on Activity page** — `Opus` — add a pipeline stage filter chip/dropdown to the Activity page so users can see all activities for contacts currently in a given stage. Backend: `GET /activity?pipeline_stage_id=` joins `contact_pipeline_entries` to filter `activity_events`. Frontend: stage picker above the activity feed (reuse the stage color chips from the pipeline board).
...
### Go-live prep
- **[GOLIVE-CHECKLIST] Write the go-live checklist** — `Sonnet` — produce a complete pre-launch checklist for 2026-06-28: Railway env vars (INBOUND_EMAIL, SECRET_KEY, ANTHROPIC_API_KEY, EVOLUTION_API vars), DNS + Cloudflare verified, Resend domain verified, superadmin account on prod, `alembic upgrade heads` + `seed.py` + `promote_superadmin.py` confirmed, module toggles set per tenant, smoke-test script (login, send mail, create ticket, check inbox). Output as `GOLIVE.md` in the repo root. **Do this before starting any other go-live work.**

### Contacts & companies
- **[LABEL-INLINE] Inline label creation from contact view** — `Sonnet` — Label management was removed from the Settings page (redundant); the only place labels can currently be created is now gone. Add an inline "Create label" option to the `LabelPicker` dropdown on `ContactDetail`: an "＋ New label" row at the bottom that expands an inline name + colour picker and calls `POST /contacts/labels`. No page navigation needed. Files: `LabelChip.tsx` (`LabelPicker` component).
...
- **[LIVECHAT-QR-ICON] QR code behind icon button** — `Sonnet` — Replace the always-visible QR panel with a small QR icon button. Pressing the icon opens a modal or popover showing the QR code to scan. QR should not be visible on the page by default. File: `ChatPage.tsx`.
- **[LIVECHAT-MEDIA] Send / paste / attach media in chat** — `Sonnet` — Users should be able to: send image files via file picker, paste images directly from clipboard into the chat input, and attach documents (PDF, DOCX, etc.). Upload to Evolution API's media send endpoint; display image thumbnails inline in the chat thread, documents as a labelled file chip. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/router.py`.
- **[LIVECHAT-STATUS] Message delivery status** — `Sonnet` — Show per-message delivery indicators in the chat thread (e.g. single tick = sent, double tick = delivered, blue ticks = read). Source the status from Evolution API's message-update webhook events (`message.update` / `messages.upsert` with `status` field). Store `status` on the `ChatMessage` model, push updates over the existing WebSocket, render tick icons next to each outbound message. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/` (models, router, websocket).
- **[LIVECHAT-TICKET] Create ticket from chat session** — `Sonnet` — One-click button in the active session panel that creates a ticket pre-filled with visitor name, linked contact, and first message as description. POST to `/tickets` with `contact_id` and `source="chat"`. Link back on the session (`ChatSession.ticket_id` already exists as a FK). Show a chip in the session panel linking to the created ticket. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/router.py`.
- **[LIVECHAT-CANNED] Canned responses in reply box** — `Sonnet` — Type `/` in the chat reply input to open a picker of saved reply templates (reuse existing `Template` model or a new `canned_responses` table). Selecting one inserts the text into the reply box. Files: `ChatPage.tsx`; backend can reuse `GET /templates`.
- **[LIVECHAT-FILTER] Session list filters** — `Sonnet` — *(filter tabs are subsumed by [LIVECHAT-STATUS-MODEL] in Tier 2; close-session history view still applies)* — Add a "Closed" history tab to the session sidebar so agents can browse past conversations. Files: `ChatPage.tsx`, `GET /chat/sessions` query param `?status=solved`.  
- **[LIVECHAT-HISTORY] Contact chat history** — `Sonnet` — When a session has a linked contact, show a collapsible "Previous conversations" section in the session panel listing past closed sessions for that contact. Clicking one loads its messages read-only. Files: `ChatPage.tsx`, `GET /chat/sessions?contact_id=&closed=true`.
- **[LIVECHAT-NOTES] Internal agent notes on session** — `Sonnet` — Notes tab alongside Messages in the active session view for internal notes not sent to the visitor. Store as `ChatMessage` with `sender_type="note"` (renders differently in the thread, never dispatched to WhatsApp). Files: `ChatPage.tsx`, `router.py`.
- **[LIVECHAT-PERF] Performance: virtual session list + WS heartbeat** — `Sonnet` — Two improvements: (1) virtualise the session list (`react-window`) so large session counts don't paint all rows; (2) add a WebSocket ping/pong heartbeat (30 s) on client and server so dropped connections are detected and auto-reconnected instead of going silently stale. Files: `ChatPage.tsx`, `router.py` WebSocket handler.
- **[LIVECHAT-CONTACT-POPUP] Click contact name → inline popup (not page navigation)** — `Sonnet` — The contact name shown at the top of an active chat session should be clickable. Clicking it opens a contact-info popup/modal (same fields as ContactDetail: name, phone, email, company, labels, recent tickets) — it must NOT navigate away from the chat page. Reuse the `GET /contacts/{id}` response and the existing contact-detail component as a modal. Allow editing from the popup (PATCH /contacts/{id}). Files: `ChatPage.tsx`, reuse `ContactDetail` or extract a `ContactPopup` component.
- **[LIVECHAT-BOOKING-EMAIL] Booking via chat requires email — prompt if missing** — `Sonnet` — When an agent sends a booking link from a chat session and the linked contact has no email address on file, show a prompt before dispatching: a small modal asking for the contact's email. On confirm, `PATCH /contacts/{id}` to save the email, then proceed with `POST /booking/send`. Without an email the booking confirmation cannot be delivered to the customer; the prompt prevents a silent failure. Files: `ChatPage.tsx`, `SendBookingModal.tsx`.
- **[LIVECHAT-CREATE-CONTACT] Create contact from unknown WhatsApp number** — `Sonnet` — When a session has no linked contact (the sender is an unknown number), show a "Create contact" button in the session panel header. Clicking opens a pre-filled "New contact" modal with the phone number already populated. On save (`POST /contacts`) the new contact is immediately linked to the session (`PATCH /chat/sessions/{id}` with `contact_id`). The session panel updates to show the contact name + a link to the contact popup. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/router.py`.
- **[LIVECHAT-SESSION-ACTIONS] Right-click + bulk actions on sessions: close / reopen / delete / broadcast** — `Sonnet` — Two interaction surfaces: (1) **Right-click context menu** on any session row in the sidebar list — options: Close, Reopen, Delete (with confirm). (2) **Multi-select checkboxes** — a checkbox appears on hover on each row; a sticky bulk action bar appears when ≥1 session is checked, with buttons: Close, Reopen, Delete, Broadcast message (opens ComposeModal pre-filled with the contacts from selected sessions). Backend: `POST /chat/sessions/bulk` with `action` (close|reopen|delete) + `session_ids`. Session status field: extend `ChatSession.status` enum to include `closed` / `open` if not already there. Files: `ChatPage.tsx`, `router.py`, `ChatSession` model.
- **[LIVECHAT-PHONE-NORM] Normalize Contact.phone to E.164 on save** — `Sonnet` — Contact phones stored without a country code (e.g. `0612345678` Dutch local) won't match inbound Evolution API webhooks (which always produce `31612345678`). Add E.164 validation/normalization on `POST/PATCH /contacts`: strip non-digits and require a country-code prefix (reject or prompt for local-format numbers). Display a warning in the UI if a contact's saved phone looks local-format. Files: `contacts/router.py`, `contacts/service.py`, `ContactDetail.tsx`, `EditContactModal.tsx`.
- **[LIVECHAT-SESSION-DEDUP] Partial unique index to prevent duplicate open sessions** — `Sonnet` — Concurrent inbound webhooks for a new number can both pass the SELECT and each create a new open session (no DB-level guard). Add a partial unique index on `chat_sessions(tenant_id, whatsapp_phone) WHERE is_open = TRUE` so the second insert fails and can be retried safely. Also add the same guard on `visitor_id` for websocket sessions: `(tenant_id, visitor_id, source) WHERE is_open = TRUE`. Migration only — no code change. Files: migration only.
- **[LIVECHAT-WIDGET-HISTORY] Widget reconnect restores message history** — `Sonnet` — Now that the widget uses `localStorage` a visitor who reloads the page reconnects to their existing open session but sees an empty chat and the hardcoded greeting replayed. On WebSocket connect, fetch the session's message history (`GET /chat/sessions/{id}/messages`) and render past messages before showing new ones. The widget needs a REST call on connect (session ID is already stored in localStorage). Files: `widget.js`, optionally expose a `GET /chat/public/sessions/{visitor_id}/messages` endpoint if a public REST route is needed (vs. requiring auth).
- **[LIVECHAT-EVOLUTION-LICENSE] Evolution API license compliance notice** — `Sonnet` — Evolution API (Apache 2.0 + additional conditions) requires that any system using it displays a **visible notification to system administrators** that Evolution API is in use, accessible from the system's settings page or documentation. Add a "Powered by Evolution API" attribution notice to the LiveChat settings/configuration section in the superadmin Settings page (or wherever chat is configured). The notice should link to `http://www.apache.org/licenses/LICENSE-2.0` and state that the live chat feature is powered by Evolution API (licensed under Apache 2.0). This is a compliance requirement — do not skip. File: wherever the LiveChat / WhatsApp configuration UI lives (likely `ChatPage.tsx` or a chat settings component in `SuperadminsSettingsPage.tsx`).
