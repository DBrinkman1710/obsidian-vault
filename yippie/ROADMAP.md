...
**Features — calendar:**
- ~~**[CAL-SPLIT] Calendar: shared vs personal events + booking from personal email**~~ — `Sonnet` — ✅ done (d871c18). `calendar_type` column + All/Shared/Personal toggle in CalendarPage; Visibility radio in EventModal; `from_email` on booking_tokens; "Send from my email" checkbox in SendBookingModal.
...
### Calendar
- ~~**[CAL-SPLIT] Shared vs personal calendar events + booking from personal email**~~ ✅ **DONE (d871c18)** — `calendar_type` column on `calendar_events` + All/Shared/Personal filter toggle in CalendarPage; Visibility radio in EventModal; `from_email` on `booking_tokens`; "Send from my email" checkbox in SendBookingModal.
