...
### AI / automation
- **[AI-BTN1] Explicit "Generate" button instead of auto-AI** — `Opus` — *inbox part DONE.* Inbox AI no longer auto-runs on thread arrival: drafts ingest with `ai_status='done'` unless the tenant opts into auto-scan. Agents click **Generate AI suggestion** per draft (existing `/inbox/drafts/{id}/generate`); briefing + scan fill in on demand with a spinner + sonner toast. Per-tenant `Tenant.ai_auto_scan` boolean (default False) gates the old auto behaviour — editable in SuperAdmin Info tab + surfaced on `/tenant/config`; migration `ai_btn1_001`. Compose suggestions were already button-triggered. **Still open:** the interactive demo tool on `getyippie.com` (paste copy, pick action type, preview campaign button) — coordinate with [WEB1].
