...
### LiveChat
- **[LIVECHAT-INBOUND] Inbound WhatsApp messages not received** — `Sonnet` — ⚠️ ROOT CAUSE KNOWN: the webhook endpoint exists on Yippie (`POST /api/v1/chat/webhooks/{slug}/whatsapp`) but `_ensure_instance` in `whatsapp_service.py` never registers this URL with Evolution API, so Evolution has nowhere to POST incoming messages. Fix: after creating the instance, call `POST /webhook/set/{instance_name}` on Evolution API with `{"url": "https://{env}.getyippie.com/api/v1/chat/webhooks/{slug}/whatsapp", "webhook_by_events": true, "events": ["messages.upsert", "message.update", "connection.update"]}`. The `PUBLIC_BASE_URL` (or `ENVIRONMENT`→URL mapping) must be an env var so the webhook URL resolves correctly per Railway env. Files: `whatsapp_service.py` (`_ensure_instance`), `config.py`.
