...
- **[LIVECHAT-ASSIGN] Assign session to agent** — `Sonnet` — Add `assigned_to` UUID FK (to users) on `ChatSession` + migration. Agent picker in the session panel; filter tabs in the session list: Mine / All / Unassigned. Browser notifications only fire for the assigned agent (or all agents when unassigned). Files: `ChatSession` model + migration, `ChatPage.tsx`, `router.py`.
...
- **[LIVECHAT-FILTER] Session list filters** — `Sonnet` — Add filter tabs to the session sidebar: All / Open / Closed / Mine. Currently shows all open sessions with no way to view closed history or filter to your own. Files: `ChatPage.tsx`, `GET /chat/sessions` query params (`status`, `assigned_to`).
