...
- **[WEB-COMPARE] More comparison pages** — `Sonnet` — *from the ux-habits + SEO/GEO audit 2026-07-09.* Clone the /vs-zendesk template (comparison table + direct answer paragraph + metadata) for /vs-freshdesk and /vs-front. Comparison pages are the highest yield SEO + GEO surface (AI assistants cite them for "X alternative" questions). Add each new page to sitemap.ts and llms.txt key pages.
- **[WEB-BLOG-NL] First Dutch blog post** — `Sonnet` — *ties into the pending blog content plan (see project notes).* One NL post targeting "klantenservice software voor MKB"; reuse the existing post scaffold (metadata + Article + BreadcrumbList JSON-LD, sitemap entry with real date). Wait for Diederik's answers to the 4 open blog questions first.
- **[WEB-PROOF] Social proof section** — `Sonnet` — **blocked on real quotes from Diederik** (sandbox test clients, Ndugu pilot?). Landing pages currently have zero testimonials/logos — the single biggest trust gap vs the SaaS landing page standard (uxpeak video 3). Build the section component when 2–3 real quotes exist; never fabricate.
- **[WEB-LOGO-CARRY] Persist onboarding logo into the tenant** — `Sonnet` — the /custom mini workspace preview (shipped 2026-07-09) shows an uploaded logo client side only (object URL). Carry it through: stash as data URL, upload to the tenant's `logo_url` after account creation/provisioning (branding endpoint from session 46 already exists). The picked colour already flows via `questionnaire.branding_color`.
- **[WEB-FEATURE-GROUPS] Homepage feature grouping** — `Sonnet` — the homepage lists 15 identical feature rows (flat hierarchy wall). Group into 3–4 categories (Support, Sales & growth, Operations, Insights) with visual hierarchy per uxpeak video 2.
- **[WEB-FOUNDER-COUNT] Live founder spots counter** — `Sonnet` — founder banner says "first 5 spots" with no live count. Add "X of 5 spots left" from a config constant (manual) or a public endpoint counting founder plan tenants (automatic). Loss aversion only works when the number moves.
- **[WEB-PRICE-DEFAULT] Pricing page smart default** — `Sonnet` — pre-select team size "4–10" in the pricing questionnaire (`pricing/page.tsx` teamSize state) so the recommendation panel renders a pre-computed result immediately (uxpeak video 1, smart defaults). The rest of the pricing psychology work (real € saved on annual, "everything in X plus" tier diffs, price emphasis) is already specced as [UX-CRAFT] Batch 3 (H) — build together.
