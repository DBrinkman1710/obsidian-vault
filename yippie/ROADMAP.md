...
- **[ACTIVITY2] Activity board — tabs + depth** — `Opus` — *not built (planned 2026-07-13).* Split the single Activity page into sub-tabs (**Overview / Users / Automation**): keep the aggregate KPIs on Overview, move the current per-user "Agent performance" section off Overview into a deeper **Users** tab, and add an **Automation** tab. Naming: humans are **users**, not "agents" — rename the existing "Agent performance" / "All agents" copy (`ActivityFeed.tsx`, `get_agent_kpis`) when touched. Users + Automation tabs are admin/manager gated (EU worker monitoring sensitivity — never expose a per person leaderboard to rank and file). Cheapest first: Phase 1 → 2 → 3 → 4. **Phased:**
  - **Phase 1 — Users tab** (`Opus`) — ✅ **shipped 2026-07-13** (migrations `act2_draft_opened_by`, `act2_thankyou_tags`): tabbed Overview/Users board, per user hero metrics (first response, first time right, avg resolution, open workload), time to open shared+personal, emails/open rate, chats, per user timeline, department rollup; **dashboard habits** — Sparkline micro charts, team comparison bar chart, week over week delta arrows; **first time right** via reopen signal (`resolved_at` set but now open) refined by the thank you tag (draft reject reason + ticket resolve toggle, excluded from the FTR denominator). Original spec below. Move + deepen the per-user view. **Layout:** stacked, no scope toggle — a **per user dashboard on top**, **department statistics below**. Two scopes only (user + department); the shared inbox *is* the department. Admin/manager gated (EU worker monitoring — no leaderboard to rank and file). **Metric set (v1):** (a) **emails received this week, shared vs personal** — `inbound_messages.email_account_id` → `email_accounts.user_id` (NULL = shared mailbox, set = that agent's personal mailbox); (b) **avg time to open, shared + personal** — ✅ shipped (migration `act2_draft_opened_by`): `draft_tickets` already stamps `opened_at` on first open; added `opened_by` and compute `opened_at − inbound.received_at` attributed to the opener, split shared vs personal via `email_accounts.user_id`. Personal time = "opened in Yippie", not read in the native client — a footnote, not a blocker (user confirmed do it for both); (c) **avg response time** — first non internal `ticket_comment` by the user − ticket `created_at`, attributed to `ticket_comments.author_id` (works on shared threads many people touched); (d) **total tickets created** — `tickets.created_by`; (e) **first time right** — measured via a **human thank you tag**, *not* a classifier: add a draft **reject reason** (`thank you | spam | duplicate | no action`, today reject is a bare binary in `inbox/schemas.py`) + an optional **thank you tag on resolve**; FTR = agent replied and the customer either did not come back or came back only with a thank you tagged mail / no reopen. This labeled data also feeds Phase 4 (mail→ticket funnel) and tells [CSAT1] not to survey a thank you close; (f) **live workload** — open + in progress tickets currently assigned; (g) **live chat handling** — chats handled + chat response time (`chat_sessions.assigned_to`, `chat_messages` agent vs preceding visitor); (h) **CSAT** per agent once [CSAT1] ships. **Trend/shape (items 6/7/8 from the design chat):** period toggle 7/30/90d + week over week deltas (reuse the [UX-DASH] sparkline pattern), module mix per user (`activity_events` by module), activity by hour + `last_login_at` presence chip. **Centerpiece:** per user timeline — the `actor_id` filtered `activity_events` stream grouped by day. **Backend:** add `actor_id` filter to `GET /activity` (`list_events` already filters `contact_id`/`pipeline_stage_id`); new per user + per department stats endpoint (FRT, time to open, workload, chat, emails received split); department rollup via `department_members`. **Two small instrumentation adds only** (no big migration): `inbound_messages.opened_at`+`opened_by`, and the reject reason + resolve thank you tag. **Deferred:** `%` incoming mail → ticket (→ Phase 4 / [CLUSTER1]); periodic relationship NPS (→ [CSAT1] v2).
  - **Phase 2 — Flows performance** (`Sonnet`, cheapest — data ready). Per flow health + ROI straight from the existing `flow_runs` audit table (already indexed `ix_flow_runs_tenant_flow_created`): fires over time, success/partial/failed/skipped rates, `error` surfaced so silently broken flows get caught, weakest step from the `results` JSONB, and est. hours saved. New endpoints: `GET /flows/{id}/stats` + a tenant rollup.
  - **Phase 3 — Yip assistant activity** (`Opus`). What the AI did / hours saved. Prereq: a first class `source` field on `activity_events` (`human|ai|automation|system`), backfilled from today's soft `payload.source="jarvis"` heuristic; audit the 12 `log_event` callers so flows / SLA / inbox auto drafts all tag consistently (right now Yip actions log under the invoking human's `actor_id`).
  - **Phase 4 — "What to automate" = [CLUSTER1]** (`Opus`). Recurring ticket clustering surfaced as automation recommendations inside the Automation tab — this is where [CLUSTER1] lands, as the maturation of phases 2–3 (flows/AI show ROI *captured*; clustering shows ROI *still on the table*).
- **[CSAT1] Customer service review email** — `Opus` — *not built (planned 2026-07-13).* After a customer's issue is handled, email them a short request to **review the service received**; they click a rating on a hosted page; the score is stored and attributed to the handling user, then surfaces as **CSAT per user + per department on the Users tab** ([ACTIVITY2] Phase 1 metric h) and as the trustworthy **first time right** signal. **Shape:** a built in flow (FLOW8 platform automation pattern) — **toggleable, logic not editable, the email template IS editable per tenant**. **Rating capture:** public hosted page `/review/{token}` (booking `/sign/:token` token pattern), 1–5 stars + optional comment, writes to a new tenant scoped RLS `csat_responses` table linking `ticket_id` + handled_by `user_id` + `contact_id`. **Trigger policy (the load bearing decision, settled):** transactional CSAT — fires on **ticket resolved / chat solved**, but **skip** thank you tagged closes and zero agent reply / auto closes (reuses the [ACTIVITY2] thank you tag), with a **per contact cooldown (default 30 days)** and optional sampling `%` so a customer is asked at most once a month after a real resolved interaction — this answers "not after every close, not for all contacts". **Deferred to v2:** periodic relationship NPS mode; routing happy raters to a public Google/Trustpilot review (review gating). Buildable now — the Flows engine + token page pattern already exist; only the table, the hosted page, and the built in flow + trigger guards are new.
...
- **[KAN_FLOW1] Kanban flowchart tab** — `Opus` — **built 2026-08-05, pending pytest/tsc + commit + sandbox verify** (Bash classifier was down during the session). Board | Flowchart tabs on the Kanban page. @xyflow/react canvas with StageNode (stage colour, inline rename/delete syncs the REAL stage), DecisionNode (yes/no diamond), start/end pills, editable arrow labels (free condition text), unplaced stage tray, "start from your stages" empty state, debounced autosave. One chart per tenant: `pipeline_flowcharts` table (JSONB graph, tenant RLS, migration `kanflow1_pipeline_flowcharts`, down_revision `tmpl1_template_blocks` — verify single head before push). GET/PUT `/pipeline/flowchart` with server side reconciliation (board stays source of truth for stages). AI context first: the chart documents pipeline semantics; execution stays in Flows.
- **[KAN_FLOW2] Flowchart feeds Yip + automation suggestions** — `Opus` — **built 2026-08-05, pending same verification as [KAN_FLOW1]** (depends on it). `flowchart_context.py` serializes the chart to compact text; injected into Yip's system prompt (`jarvis/agent.py`) as "How this tenant's pipeline works" only when the pipeline module is on and a non empty chart exists. GET `/pipeline/flowchart/suggestions` deterministically maps chart edges (incl. decision yes/no branches) to draft automations: trigger `pipeline_stage_changed` + `stage_id equals SOURCE` condition + `move_pipeline_stage` action prefill. "Suggest automations" button on the flowchart tab → modal of suggestion cards → "Create in Flows" navigates to /flows with `location.state.flowPrefill`, opening the existing BuilderModal seeded (normal create path, no parallel mechanism).
- **[CLUSTER1] Issue cluster generator** — `Opus` — *not built* (no `ticket_clusters` table or insights code). Spec above. Note: no longer hard-blocked on [AI-MOD1] — Mistral/Anthropic fallback works for early tenants. **Now folded into [ACTIVITY2] as Phase 4** ("what to automate" surface inside the Automation tab) — build it there.
...
### Skim index — one line each; full detail in the entries below (older history in Appendix A)

| Date | Item(s) | What shipped |
|---|---|---|
| 2026-08-07 | [YIP-KB] | Yip learns your Q&A page — per tenant FAQ URL fetched + chunked (kb_source/kb_chunk, RLS), keyword retrieval grounds suggest-reply drafts, "Yip knowledge" settings card |
| 2026-07-10 | [WEB-COMPARE] + 5 web | Marketing batch: /vs-freshdesk + /vs-front, Dutch /nl site, homepage feature groups, founder spots counter, smart price default, onboarding logo carry |
| 2026-07-10 | [FLOW-CANVAS-DD] | Drag and drop flow canvas (node palette + saved positions) |
| 2026-07-10 | [UX-PSYCH] | UX psychology overhaul — 6 items (micro animations, empty states, closure toasts, hotkey overlay, nudge badge, undo toasts) |
| 2026-07-10 | [YIP-FLOW] | Yip builds/lists/explains flows from chat (admin gated) |
| 2026-07-09 | [WEB-MINI] + [WEB-SEO1] + [WEB-GEO1] | Mini Yippie preview on /custom + SEO/GEO batch (JSON-LD, llms.txt, AI crawler allows) |
| 2026-07-09 | [FLOW8] | Built in automations catalogue + /flows surface; retired 4 hardcoded jobs |
| 2026-07-08 | [UX-CONV] | Conversion psychology batch (smart defaults, endowed progress, anchoring, loss aversion) |
| 2026-07-08 | [TRIAL30] | 30 day free trial on signup + nudge/expiry scheduler |
| 2026-07-08 | [FLOW2A–D] | Flows engine phase 2: delay/retry, OR groups, time triggers, saas_health_dropped |
| 2026-07-08 | [FLOW5] | Flows webhook triggers + actions (SSRF pinned, HMAC signed) |
| 2026-07-08 | [FLOW3] + [FLOW4] | Visual flow canvas + if/else branching (DAG) |
| 2026-07-08 | [FLOW-POLISH] + [FLOW-CANVAS-NEW] | Flows QoL (test fire, duplicate, run stats) + new flow on canvas |
| 2026-07-07 | [FLOW1] | Flows module phase 1 — When/If/Then rule engine + outbox |
| 2026-07-06 | [YIP4] + [YIP5] + [YIP-STREAM] | Yip upgrade: 6 read tools, server threads, SSE streaming, morning briefing |
| 2026-07-06 | [CONTRACT2] + [CONTRACT3] | Contract lifecycle (MRR/ARR, renewals) + draw to sign e-signing |
| 2026-07-06 | [EXT1-STORE] · [EML1] · [CALL1] | Chrome store submission · Gmail/Outlook OAuth linking · click to call + recording |
| 2026-07-06 | [Phase 11C T2] | "Connect your inbox" ROI estimate |
| 2026-07-05 | [YIP2] + [YIP3] + [YIP-GATE] | Yip drafts replies in tenant tone, write actions w/ confirm chips, module gated tools |
| 2026-07-05 | [CTX1] | `customer_context.py` — canonical full history aggregator feeding all AI surfaces |
| 2026-07-05 | [CONTRACT1] | Contracts module phase 1 (storage + up/download) |
| 2026-07-04 | [YIP-AGENT] | Yip rebuilt as agentic tool loop + `assistant_memories` |
