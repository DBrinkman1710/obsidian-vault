...
## ⚡ Flows initiative — [FLOW1] ✅ shipped, phases 2–6 planned (2026-07-07)
...
**[FLOW2A] Engine core: delay steps + retries** — `Opus` — *run FIRST: its single migration `flows2_phase_two` (chained from `flows1_flow_tables`) carries ALL phase 2 DDL so the other three sessions are migration free (no parallel head races).*
...
**[FLOW2B] OR condition groups** — `Sonnet` — *independent of 2A, no DDL.*
...
**[FLOW2C] Time triggers: schedule + ticket_sla_due_soon** — `Sonnet` — *needs [FLOW2A]'s migration columns (trigger_config, last_scheduled_on, sla_flow_emitted_at).*
...
**[FLOW2D] saas_health_dropped trigger** — `Haiku` — *independent, no DDL, small and fully specified.*
- In `compute_health_for_contact` (`app/modules/saas/service.py`, upsert around line 124): capture the pre overwrite `row.score`; if the row pre existed and new < old → `emit_flow_event(db, tenant_id, "saas_health_dropped", entity_type="contact", entity_id=contact_id, contact_id=contact_id, payload={"score", "previous_score", "drop"})`. Flush only — the hourly job's per contact commit carries score + event together (outbox guarantee). **No previous_score column needed**; "once per threshold crossing" is expressed in conditions (`score lte X AND previous_score gte X+1`) and a flat low score never re emits.
- `TRIGGER_META["saas_health_dropped"]` with module `"saas"` (builder/meta/validation gating is automatic via the module key) + condition fields score/previous_score/drop (number).
...
### Phases 3–6 — next (directional specs, refine at session start)

**[FLOW3] Visual flow canvas** — `Opus` — *after phase 2.* The visual mapping layer: a node canvas per flow using `@xyflow/react` (React Flow) at `/flows/:id`, keeping the modal builder for quick edits. Nodes: trigger (with schedule config), one node per OR condition group, action, wait — auto laid out top to bottom (dagre), edges labelled and/or. **Edit parity:** canvas mutations write the exact same flows JSON as the modal (no schema change this phase). **Run replay:** pick a run from the drawer → highlight the path taken with per node ok/skipped/failed/attempts badges from `flow_runs.results`. Stretch: a tenant wide "connections" overview mapping every enabled flow as trigger module → action module edges — the "how is my business wired" picture.

**[FLOW4] Branching paths (if/else inside a flow)** — `Opus` — *after [FLOW3] (canvas is the editing surface for branches).* Generalize `actions` from a list to a small DAG: `{"nodes":[{id,type,config}],"edges":[{from,to,when:"match"|"else"|null}]}` with a `branch` node type holding condition groups; legacy linear list = single chain via a normalizer (same no migration trick as [FLOW2B]). Engine walks the graph; waits/retries reuse `flow_pending_steps` (pending step stores next node id + graph snapshot instead of a remaining list). Schemas validate the DAG (single root, no cycles, ≤ 25 nodes). Canvas branch node gets match/else output handles; the modal builder goes read only for branched flows.

**[FLOW5] Webhook triggers & actions** — `Sonnet` — *after phase 2, independent of [FLOW3]/[FLOW4].*
...
**[FLOW-POLISH] Flow QoL batch** — `Haiku` — *after phase 2.* Test fire button (synthesize a sample event for the trigger, dry run: evaluate conditions + list which actions would run, execute nothing), duplicate flow, per flow success/fail counts on the list row (from `flow_runs`), status filter in the runs drawer.
...
- **[FLOW2A] Flows engine core: delay steps + retries** — `Opus` — run **first** of the phase 2 sessions; carries the single `flows2_phase_two` migration (pending steps table + all phase 2 columns). Plan file: `~/.claude-account2/plans/build-phase-2-of-sequential-pelican.md`.
- **[FLOW3] Visual flow canvas** — `Opus` — after phase 2; React Flow node canvas per flow, edit parity with the modal builder, run replay highlighting.
- **[FLOW4] Flow branching (if/else paths)** — `Opus` — after [FLOW3]; actions list → validated DAG with branch nodes, engine walks the graph.
...
- **[FLOW2B] Flows: OR condition groups** — `Sonnet` — independent of [FLOW2A], no DDL; spec in the Flows initiative section.
- **[FLOW2C] Flows: time triggers (schedule + ticket_sla_due_soon)** — `Sonnet` — needs [FLOW2A]'s migration columns; spec in the Flows initiative section.
- **[FLOW5] Flows: webhook triggers & actions** — `Sonnet` — after phase 2; tokened inbound hook URL + HMAC signed outbound webhook action with SSRF guard.
- **[FLOW6] Flows: flow chaining** — `Sonnet` — after phase 2; chain_depth ≤ 3 + cycle guard replaces the blanket `source="flow"` skip, opt in per flow.
...
- **[FLOW2D] Flows: saas_health_dropped trigger** — `Haiku` — independent, no DDL; emit on score drop in `compute_health_for_contact` + TRIGGER_META entry; spec in the Flows initiative section.
- **[FLOW-POLISH] Flows QoL batch** — `Haiku` — after phase 2; test fire (dry run), duplicate flow, run stats on the list row, runs drawer status filter.
