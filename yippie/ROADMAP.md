...
- **[CONTRACT3] Contracts phase 3: generation + e-signing** — `Opus` — contract templates (mirror the campaign TemplatePicker) with company/contact merge fields → render to PDF. Public `/sign/:token` page reusing the booking-token pattern (`booking_tokens` → `/book/:token`); capture signature + `signed_at`/`signer_name`/`signer_ip` audit trail. For legal-grade signing, later swap the in-house capture for an eIDAS integration (SignRequest NL or Zoho Sign). On signature, optionally kick off a Stripe Connect subscription from the contract's value/interval — Layer 2 of the Stripe plan. May justify an e-sign price bump; revisit at build time.

---

## 🔵 Tier 2 — Open (verified against code 2026-07-04)

- **[CONTRACT1] Contracts module — phase 1: storage** — `Sonnet` — *built locally 2026-07-05, pending deploy + sandbox verify.* New €9/mo `contracts` module: `contracts` table (title, type, status, `direction` issued|received, company/contact counterparty, tags, notes, inline BYTEA document) + RLS; backend CRUD + file up/download at `/contracts`; frontend list with status inline-edit, direction filter, add/edit peek modal, company+contact pickers, PDF/Word/image upload. Registered in modules.json (`pnpm sync:config`), MODULES registry, migrations/env, both SuperAdmin module lists, Sidebar, i18n. Move to ✅ Done once verified on sandbox.
- **[CONTRACT2] Contracts phase 2: lifecycle + renewal reminders** — `Sonnet` — add `start_date`, `end_date`, `notice_period_days`, `auto_renew`, `renewal_term`, `value_amount`, `value_interval` (one_off|monthly|yearly), `currency`. Derived `notice_deadline = end_date − notice_period_days`. APScheduler job (model on `demo_expiry_check`/booking jobs) fires a deadline toast + Yip proactive nudge before notice deadline / expiry. "Renewals" view (expiring soon / auto-renewing / expired). Roll up sum of active `value_amount` per tenant as live MRR/ARR feeding the Billing module. Design value/interval fields so Stripe can consume them in phase 3.
