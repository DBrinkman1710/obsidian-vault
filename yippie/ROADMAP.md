...
- **[MKTG-EXPAND] In-app Marketing module: expand feature depth** — `Sonnet` — The Marketing module (session 69) has campaigns, templates, A/B testing, drip sequences, and basic analytics. User wants richer functionality — see Tier 2 for the full expansion spec. Templates + campaign buttons should live fully inside this module (not split with `/settings/templates`). Activity card should show open rate + response rate prominently. See Tier 2.
...
### Marketing module
- **[MKTG-EXPAND] Marketing module: expand feature depth** — `Sonnet` — Current state (session 69): campaigns, GrapesJS designer, A/B testing, drip sequences, Resend + Evolution dispatch, open/click/reply tracking, unsubscribes. Gaps + new features to add: **(1) Templates live in Marketing** — move campaign template management fully into `/marketing` (design tab already has GrapesJS; remove the split with `/settings/templates` for campaign-type templates, or clearly delineate); **(2) Campaign buttons** — ensure campaign-button blocks are accessible directly inside the GrapesJS Marketing canvas, not only via `/settings/templates`; **(3) Activity card** — the marketing sidebar overview must prominently show open rate + response rate for the last 30 days (bar chart or KPI cards); **(4) Test-send** — "Send test to myself" button before launching a campaign, dispatches to the agent's own email so they can check rendering; **(5) Personalization tokens** — `{{first_name}}`, `{{company}}`, `{{email}}` replaced at send time per recipient (inject into both HTML and plain-text versions); **(6) Campaign duplication** — "Duplicate" button on campaign list to copy design + settings as a new draft; **(7) Bounce handling** — inbound Resend `email.bounced` webhook auto-adds contact to a `contact_bounces` table; bounced contacts are skipped on future sends; bounce count shown on analytics; **(8) Contact engagement score** — scalar 0–100 per contact: +5 open, +10 click, +15 reply, −10 opt-out, decays monthly; shown as a badge on ContactDetail + filterable in segment builder; **(9) Unsubscribe management page** — tab inside Marketing listing opted-out contacts with date + campaign that triggered it, with a "Re-enable" button (with confirmation). Files: `pages/marketing/MarketingPage.tsx`, `modules/marketing/service.py`, `modules/marketing/router.py`, `modules/marketing/schemas.py`, `emailtracking/webhooks.py`. See Tier 2.
...
- **[RC1] Right-click context menus** — `Sonnet` — *5 of 8 pages done (verified session 76).* `ContextMenu.tsx` shared component exists and is wired into ✅ TicketList, ✅ ContactList, ✅ InboxQueue, ✅ ChatPage, ✅ InvoiceList. Still missing: **PipelinePage** (Move to stage, Send booking link, Remove from kanban), **CalendarPage** (Edit/Delete event, New event on day cell), **TemplatesPage** (Edit, Duplicate, Delete). Files: `pipeline/PipelinePage.tsx`, `calendar/CalendarPage.tsx`, `admin/pages/TemplatesPage.tsx`.
...
- **[COM-NAV] Commercial site: header nav misaligned with page content** — `Haiku` — `padding: 0 6vw` on `.nav` vs `max-width: 1180px; margin: 0 auto` on sections — they don't align at most viewport widths. Wrap the nav inner row in the same 1180px centred container. Files: `SiteChrome.module.css`, `SiteNav.tsx`. See "New items collected 2026-06-21".
### Contacts (session 76+)
- **[CON-TABS] Contacts: swap tab order + remove Trash tab** — `Haiku` — Reorder tabs to Contacts | Companies and remove Trash tab. Files: `ContactsPage.tsx` line 782. See Tier 3.
- **[CON-DELETED] Deleted contacts in search/list** — `Haiku` — Soft-deleted contacts visible in search results and at bottom of contacts list; investigate which fetch path skips the `deleted_at.is_(None)` filter. See Tier 3.
- **[CON-PERMDEL] Delete permanently — silent failure** — `Haiku` — Non-admin users see button but get a silent 403; add error toast + hide button for non-admins. Files: `ContactsPage.tsx`. See Tier 3.

### Inbox (session 76+)
- **[INBOX-SHIFT] Assign to me + empty list causes layout shift** — `Haiku` — Add `min-h` to draft list container. Files: `InboxQueue.tsx`. See Tier 3.
- **[TK-NAV] Sidebar disappears on Tickets page** — `Haiku` — CSS height/overflow conflict causes sidebar to disappear when navigating to tickets. Files: `TicketList.tsx`, `Sidebar.tsx`. See Tier 3.

### Team (session 76+)
- **[TEAM-DEPT] Add department assignment to team member edit modal** — `Haiku` — `EditUserModal` already has email/name/role. Add department multi-select. Files: `TeamSettingsPage.tsx`, `team/router.py`. See Tier 3.
