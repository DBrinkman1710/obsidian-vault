...
[BIZ-Q1], [CUSTOM1], [B2X1], Per-tenant custom domain, [Phase 11C T2] "Connect your inbox", [LANG1], [WEB-CONS1], [DEPT-ROUTING], [INSIGHT1], Customer data + AI briefing
...
- **[DEPT-MOD] Make Departments a proper module** — `Sonnet` — add `"departments"` to `ALL_MODULES`; superadmin can toggle it per tenant; sidebar Departments link only visible when enabled. See Tier 2.
- **[DEPT-ROUTING] Department inbox routing + shared/personal toggle** — `Opus` — `departments` table already exists; extend with `inbound_email` (e.g. finance@client.com); inbound mail router matches `to` against department emails and auto-assigns `department_id` on new ticket. Users in that dept see a "Shared [Dept] Inbox". Setting per user/dept to **disable shared inbox** → shows only personally-assigned tickets (Personal Work Inbox). Large architectural change; affects `email_poller.py`, inbox routing, `InboxQueue.tsx`. See Tier 1.
...
- **[MKTG1] Marketing module** — `Sonnet` — *not built.* Dedicated sidebar module (replaces old TemplatesPage as the home for all campaign-related work). Houses GrapesJS template designer, campaign buttons, audience targeting, dispatch, and analytics in one place. **Depends on [GRAPES1] being merged.** Supersedes [CAM1].
...
- **[INSIGHT1] Tenant performance analytics** — `Opus` — *not built.* Superadmin dashboard showing per-tenant metrics: tickets open/closed/overdue, inbox response times, AI usage, feature adoption. Highlight "opportunity tenants" with low feature adoption or degrading SLAs to surface upsell suggestions.
- **[SUPER-DASH] Superuser cross-tenant activity dashboard** — `Opus` — *not built.* Dedicated superadmin view (separate panel in the superadmin area) showing live and historical activity across **all** tenants in one screen: message volume, ticket creation/close rate, AI usage, active users today, new contacts created, bounce/error spikes. Filterable by tenant, date range, and metric. Goal: spot underperforming or at-risk tenants without impersonating each one. Merges with and extends [INSIGHT1] — implement as one combined dashboard. Backend: `GET /admin/stats?start&end&tenant_id=` (superadmin-only). Frontend: "Dashboard" tab on SuperAdminPage with stat cards + per-tenant activity table. See Tier 1.
...
### Tickets
- **[MERGE-TK] Merge Tickets** — `Sonnet` — *not built.* "Merge" button on TicketDetail opens modal; agent searches for secondary ticket (by ID or subject, same contact). Backend `POST /tickets/{id}/merge` re-parents all related records (email_messages, comments, activity_events) from secondary → primary; secondary → Closed + internal note "Merged into ticket #[primary]"; primary gets activity note "Ticket #[secondary] merged in". Files: `TicketDetail.tsx`, `tickets/router.py`, `tickets/service.py`.
...
- **[SA-CLIENTS] Clients page: plan column + API token donut + clickable users column** — `Sonnet` — *not built.* Three improvements to the Clients table in SuperAdminPage: (1) **Plan column** — show each tenant's current plan (Founder/Starter/Growth/Pro) as a small badge directly in the table row, so plan distribution is visible at a glance without opening the edit modal. (2) **API token usage donut** — small donut chart per tenant showing token consumption vs. limit: neutral below 75%, orange 75–99%, red at 100%. Pulls from the `usage` table (or `tenant.token_usage` if tracked on the model). (3) **Clickable Users count** — the existing user count in the row becomes a button; clicking opens a slide-over or modal listing that tenant's users (name, email, role, last active, status) without having to open the full edit modal. Files: `SuperAdminPage.tsx`, `admin/router.py`, `admin/service.py`. See Tier 2.
...
- **[36b-6] XLSX import broken** — `Haiku` — fix `.xlsx` import (CSV works); `openpyxl` is in `requirements.txt` so likely a Railway build cache issue; force redeploy or check Railway logs.
...
### Go-live prep
- **[GOLIVE-CHECKLIST] Write the go-live checklist** — `Haiku` — produce a complete pre-launch checklist for 2026-06-28: Railway env vars (INBOUND_EMAIL, SECRET_KEY, ANTHROPIC_API_KEY, EVOLUTION_API vars), DNS + Cloudflare verified, Resend domain verified, superadmin account on prod, `alembic upgrade heads` + `seed.py` + `promote_superadmin.py` confirmed, module toggles set per tenant, smoke-test script (login, send mail, create ticket, check inbox). Output as `GOLIVE.md` in the repo root. **Do this before starting any other go-live work.**
- **[WEB-LEGAL] Privacy Policy + Terms of Service on getyippie.com** — `Haiku` — create `/privacy` (GDPR/AVG-compliant privacy statement covering data processing, retention, cookies, contact details) and `/terms` (Terms of Service covering acceptable use, liability, cancellation). Add links to both in the site footer (next to existing nav items). Without these, EU prospects stall on "is this GDPR-compliant?" before even requesting a demo — 10 minutes of work that removes a common sales blocker. Files: `apps/web/src/app/privacy/page.tsx`, `apps/web/src/app/terms/page.tsx`, `SiteFooter.tsx`. See Tier 3.
...
- **[CO-CLICK] Companies clickable → contact list** — `Haiku` — company name/badge in Contacts page links to a filtered contact list for that company; reuse `?company_id=` filter. File: `ContactsPage.tsx`.
...
- **[LIVECHAT-QR] Fix WhatsApp QR code** — `Haiku` — QR fetch fails silently in some envs; diagnose Evolution API env var wiring, improve error state in `ChatPage.tsx` (lines 228–242), ensure QR refreshes automatically when it expires. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/`.
...
- **[LIVECHAT-MEDIA] Send / paste / attach media in chat** — `Haiku` — Users should be able to: send image files via file picker, paste images directly from clipboard into the chat input, and attach documents (PDF, DOCX, etc.). Upload to Evolution API's media send endpoint; display image thumbnails inline in the chat thread, documents as a labelled file chip. Files: `ChatPage.tsx`, `apps/app/backend/app/modules/chat/router.py`.
...
- **[LIVECHAT-BOOKING-EMAIL] Booking via chat requires email — prompt if missing** — `Haiku` — When an agent sends a booking link from a chat session and the linked contact has no email address on file, show a prompt before dispatching: a small modal asking for the contact's email. On confirm, `PATCH /contacts/{id}` to save the email, then proceed with `POST /booking/send`. Without an email the booking confirmation cannot be delivered to the customer; the prompt prevents a silent failure. Files: `ChatPage.tsx`, `SendBookingModal.tsx`.
