...
[CUSTOM1], [B2X1], Per-tenant custom domain, [Phase 11C T2] "Connect your inbox", [LANG1], [WEB-CONS1], [DEPT-ROUTING], [INSIGHT1], Customer data + AI briefing, [AI-MOD1] Self-hosted AI module, [SALES-MOD1] Sales module (website tracking tag), [TRACK1] Track & trace module, [EMBED1] Lead capture embed widget
...
- **[RC1] Right-click context menus across all list/card views** — `Sonnet` — *not built.* Add a shared portal-based `ContextMenu` component (`frontend/src/components/ContextMenu.tsx`) and wire it into 8 pages. The shared component renders via `ReactDOM.createPortal` to avoid overflow-clipping (critical for Kanban columns and table rows), positions itself at the cursor, flips left/up when near viewport edges, and closes on outside mousedown / Escape / scroll. Items support normal, danger, disabled, divider, and section-label variants with Lucide icons. Each page gets `onContextMenu` on its row/card element.
  - **TicketList** — Open, Open in new tab, Set status (5 flat items: Open/In progress/Waiting/Resolved/Closed via `PATCH /tickets/{id}`), Copy link, Delete
  - **ContactList** — Open, Open in new tab, Send email (`mailto:`), Create ticket (nav to `/tickets/new?contact_id=`), Copy email, Export this contact, Delete
  - **InboxQueue** — Approve → ticket, Mark as spam (`PATCH /inbox/drafts/{id} {status:'spam'}`), Archive (`{status:'bin'}`), Delete
  - **PipelinePage** — View contact, Open in new tab, Move to stage (dynamic list from `board`, current stage greyed), Send booking link (single-contact `SendBookingModal`), Remove from kanban
  - **CalendarPage** — On event: Edit, View linked contact (disabled if none), View linked ticket (disabled if none), Delete (events only — deadlines are SLA, no delete). On empty day cell: New event on this date (pre-fills `start_at` to noon of that date)
  - **ChatPage** — Upgrade existing inline `SessionContextMenu` to shared component; add View contact, Assign to agent items
  - **TemplatesPage** — Edit (select it), Duplicate (`POST /tickets/templates` with name + " (copy)"), Delete
  - **InvoiceList** — View/Edit, Copy invoice number (sonner `toast.success`), Mark as paid / Mark as sent (`PATCH /billing/invoices/{id}`), Export, Delete
  - Files: `components/ContextMenu.tsx` (new), `tickets/pages/TicketList.tsx`, `contacts/pages/ContactList.tsx`, `inbox/pages/InboxQueue.tsx`, `pipeline/PipelinePage.tsx`, `calendar/CalendarPage.tsx`, `chat/pages/ChatPage.tsx`, `admin/pages/TemplatesPage.tsx`, `billing/pages/InvoiceList.tsx`.
