...
**Bugs — app:**
- **[BK-DATE] Booking: available dates cut off around 3 July** — `Haiku` — the booking page (`/book/:token` and `/meet/:slug`) shows no available slots after ~3 July. Root cause is likely `expiry_days` in `calendar_settings` (default too low) or a bug in `get_available_slots()` that caps the lookahead window too aggressively. Investigate both paths and raise the default lookahead to at least 60 days or make it configurable. Files: `booking/service.py`, `BookingSettingsModal.tsx`. See Tier 3.
- **[DEMO-BUG] Request demo: "Something went wrong, please try again"** — `Haiku` — `POST /public/request-demo` (or the Next.js `/api/request-demo` proxy) returns an error. Check `apps/web/src/app/api/request-demo/route.ts` and backend `public/router.py` for schema mismatches after the [BIZ-Q1] questionnaire refactor, CORS issues, or missing Railway env vars. See Tier 3.
- **[EMAILTRACK-NAV] Email tracking sidebar link opens billing page** — `Haiku` — clicking the Email Tracking (or Marketing) sidebar item navigates to the billing page instead. Check `MODULE_MAP` in `Sidebar.tsx` and the matching route in `App.tsx`. See Tier 3.
...
- **[BOOK-LOGO] Booking page shows old logo** — `Haiku` — the public booking page (`/book/:token`) and `/meet/:slug` show a stale or incorrect logo. Replace with the current logo asset or the tenant `BRANDING_LOGO_URL`. Files: `BookingPage.tsx`, `MeetPage.tsx`. See Tier 3.
...
- **[FOUNDER-PLAN] Founder plan: 10 users + 50% discount on modules** — `Haiku` — update the Founder plan: max 10 users and 50% discount on all paid add-on modules. Update `plans.py` `PLAN_LIMITS` + the Founder card on the pricing page in `getyippie.com`. See Tier 3.
