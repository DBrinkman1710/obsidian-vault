...
### Booking & scheduling
- **[BK1] Booking system** — `Fable` — *not built.* Customer-facing appointment booking. Tenant sends a booking link (via inbox / compose) to a Contact. Customer opens a public page (`/book/{token}`, no auth) showing the tenant's available calendar slots (derived from per-tenant working-hours config + existing `CalendarEvent`s). Customer picks a slot → `CalendarEvent` created in the tenant's calendar + confirmation email to customer via Resend + notification email to tenant. Requires: `working_hours` model per tenant (days, from/to times), token-gated public booking page. Builds on existing `calendar/` module.
...
- **[WEB1] Website remodel** — `Fable` — *not built.* Full redesign of `getyippie.com` (`apps/web/`). New layout, improved copy, better visual hierarchy. Coordinate with [SEO1] (meta/sitemap), [PRICE1] (pricing page), and [AI-BTN1] (demo tool on the site). Supersedes the current minimal Next.js 14 landing page.
...
### UI / UX
- **[OPT1] Optimistic UI updates** — `Opus` — *not built.* Key user actions (inbox reply send, ticket status change, contact create/edit, label apply) should update the UI immediately without waiting for the API response; roll back with a toast on error. Identify the top 5 highest-friction interactions and apply optimistic updates there first.
...
### Bugs & config
- **[ACT2] Activity page regression** — `Opus` — user reports activity page not working (despite session-42 fix). Investigate in sandbox, identify regression, fix. (session-42 fix rebuilt `ActivityFeed.tsx` with real stat cards + humanized events; if broken again, check the `/activity/stats` endpoint and `list_events()` LEFT JOIN.)
...
### Template editor
- ~~**[TE3] Campaign buttons: per-button action type** (partial)~~ — action_type selector shipped (session 42): **Apply label** (label picker, existing 9C token flow) + **Pipeline stage** (stage picker, moves contact via LabelClickToken with action_type=pipeline_stage). Still open: **open website** (URL), **send mail** (email address), **call phone number** (phone number) — these render as plain `<a>` links, no token needed; add to TemplatesPage button config when needed.
...
- **[P2] Stage changes as activity events** — `Opus` — when a contact moves between pipeline stages, write an `activity_event` row (event_type `pipeline_stage_changed`, body "Moved to {stage name}"). Hook into `_assign_stage()` in `pipeline/service.py` after the DB commit. The activity feed then shows the full history of stage transitions inline with emails, notes, and calls — no separate audit log needed.
...
- **[TK4] Tickets: customer panel in dead space** — `Sonnet` — *not built.* The ticket list/detail has empty horizontal space on the right. When a ticket is selected, show the attached Contact's card in a right-panel column: name, email, company, labels, recent ticket count. Reuse `GET /contacts/{id}`. No new endpoint needed.
- **[EMPTY1] Loading & empty states** — `Sonnet` — *not built.* (1) "All Caught Up" empty state for the inbox: minimal illustration + fun fact rotating every 2 minutes (`setInterval`, hardcoded list of ~20 facts). (2) All primary action buttons (inbox reply/send, ticket status change, compose send, contact save) must show an explicit spinner + `disabled` state while the request is in flight — no layout shift, no double-submit.
- **[TK5] "Waiting for customer" ticket state** — `Sonnet` — *not built.* New ticket status enum value `waiting_for_customer`, sits between Open and Resolved. Backend: add to `TicketStatus` enum + Alembic migration. Frontend: status dropdown, filter sidebar pill, amber colour badge. Optional: pause SLA clock while in this state (per-tenant toggle).
- **[TAG-RM] Remove tags entirely** — `Sonnet` — tags are redundant now that Kanban stages and Ticket stages cover categorisation. Check whether `tags`/`contact_tags`/`ticket_tags` DB tables exist (may never have been built). If they exist: Alembic migration to drop them. Remove any API endpoints, frontend tag components (chips, pickers, filter chips, Settings section), and sidebar links.
- **[KAN-RN] Rename "Pipeline" → "Kanban"** — `Sonnet` — update all user-visible strings: sidebar link label, page `<h1>` title, button text, toast messages, modal headers. Backend model/table names (`pipeline_stages`, `contact_pipeline_entries`, `pipeline/` module folder) stay unchanged — no migration. Update ROADMAP references after.
- **[SIDE-COL] Collapsable sidebar** — `Sonnet` — *not built.* Sidebar collapses to icon-only rail (~48 px wide) via a chevron toggle at the bottom. Collapsed state persisted in `localStorage`. When collapsed: show only icons; tooltip on hover shows the nav item label.
