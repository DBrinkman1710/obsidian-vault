...
**[FLOW6] Flow chaining (flows trigger flows)** — `Sonnet` — *after phase 2; replaces the blanket `source="flow"` skip.* Flow caused events carry a `chain_depth` in the payload; the engine evaluates them only when `chain_depth < 3` and the flow id is not already in the chain path (cycle guard, path carried in the payload). Opt in per flow (`trigger_config {"chainable": true}`, default off) so phase 1 behaviour stays the default. **Amendment (2026-07-08):** budget for more plumbing than the spec reads — executors currently emit with a bare `source="flow"` and don't carry *which* flow caused the mutation, so `chain_depth` + the cycle path means threading flow identity through all five executors' service calls into `emit_flow_event`. Still Sonnet sized, but it touches every executor, not just the engine.
...
**[FLOW7] Trigger registry: per module trigger declarations + full trigger coverage** — `Opus` — *independent of [FLOW5]/[FLOW6]; do first among phases 7–9 (both [FLOW8] and [YIP-FLOW] build on it).*
...
**[FLOW8] Built in automations — full inventarisation + /flows surface** — `Opus` — *after [FLOW7].* Clients should see every automation Yippie runs for them; where the flow engine can express it, migrate it there so clients can own and edit it.
...
- **Phase B — migrate the expressible ones:** items 1, 4, 5, 7, 8, 12 above. Each migration is: (a) add the flow trigger in [FLOW7] if not already there (`campaign_button_clicked`, `order_received`, `booking_created`), (b) install default enabled flows at tenant seed + one-time backfill for existing tenants, (c) retire the hardcoded code path in the same deploy. Critical for items 4+5: `LabelClickToken`'s `action_type/stage_id/label_id` columns become deprecated; the click handler emits the event and lets the engine handle it. Critical for item 7: remove the three `order_*_stage_id` columns from Tenant (migration + superadmin panel cleanup). Critical for item 8: remove `CalendarSettings.post_booking_stage_id`. Do items in this order: 12 first (easiest, self-contained), then 1, then 8, then 7, then 4+5 last (most complex — changes the public `/track/click` path).
...
- **[YIP-FLOW] Yip builds flows from chat** — `Fable` — *after [FLOW7].* `create_flow` write tool (propose → Confirm, created disabled, deep link to canvas) with schema generated from the trigger/action registry; `list_flows` + `explain_flow_run` read tools. Spec above.
- **[UX-PSYCH] UX psychology overhaul** — `Fable` — Six evidence-based improvements drawn from app-retention research (variable reward, progress closure, friction reduction, loss aversion, discoverability). No new backend endpoints or migrations needed — pure frontend. **(1) Micro-animations on destructive/positive actions:** resolve ticket → card slides out + checkmark pulse (CSS keyframe); move kanban card → smooth translate; archive/bin mail → swipe-away fade. Every irreversible action must have motion feedback so users feel certainty. **(2) Empty states as conversion funnels:** every empty list (no contacts, no tickets, no campaigns, no flows, no bookings) must render an illustration + a single prominent CTA button that starts the creation flow — never a blank page. **(3) Daily-work closure feedback:** after the user resolves the last open ticket or clears their inbox queue, show a sonner toast with a brief summary ("All caught up — 8 tickets resolved today"). Stat derived client-side from the session's action count; no DB write. **(4) Keyboard shortcut discovery overlay:** pressing `?` anywhere in the app opens a modal listing all active hotkeys (`j/k` navigation, `r` reply, `e` resolve, etc.) grouped by page. Pure frontend; reads the same keydown listeners already registered. **(5) Engagement nudges via sidebar badge:** if the user has not replied to any lead in the pipeline for >48 h, show a small amber dot on the Pipeline sidebar icon with a tooltip "No pipeline activity in 2 days". Computed from `last_activity_at` on pipeline stage contacts — read-only query, no new column. **(6) Inline undo toasts instead of confirm modals:** deleting a contact, label, or template should skip the modal and instead immediately delete + show a sonner "Deleted — Undo" toast (5 s window); Undo calls the restore endpoint. Reduces friction on routine ops while keeping a safety net. Sonnet is already installed (`sonner`). **Companion item: [UX-CONV]** (conversion/onboarding psychology, Tier 2) — from uxpeak video analysis 2026-07-08.
- **[TRIAL30] 30 day free trial on signup** — `Fable` — **BUILT 2026-07-08, pending pytest/tsc + sandbox verification (not yet committed).** Implementation notes: migration `trial30_trial_fields` (chains from `85f2fad0a594`, single head) adds `trial_ends_at`/`trial_nudge_sent_at`/`trial_final_nudge_sent_at`; signup stamps the trial BEFORE `set_tenant_context` (tenants RLS would block the UPDATE from root context) and now COMMITS at the end — found + fixed a latent bug where the root tenant contact/Live stage work was silently rolled back (only `create_tenant`'s own commit persisted); `handle_signup_payment` no longer called at signup; `trial_nudge_check` + `trial_expiry_check` hourly jobs (nudges at 7 days left reciprocity / 2 days left loss aversion with real contact+ticket counts, expiry deactivates unless converted); conversion clears the trial via Stripe webhook (`checkout.session.completed`, `invoice.paid`, subscription sync — ⚠️ webhook endpoint NOT yet configured in the Stripe dashboard, code path is ready) or manually when a superadmin sets `go_live_at` (update_tenant side effect; `trial_ends_at` added to TENANT_SAFE_FIELDS + TenantUpdate/TenantOut); TenantConfig exposes `trial_ends_at`; `TrialBanner` in App.tsx (brand blue, amber loss aversion in final 5 days) + trial chip on SubscriptionPage; getyippie.com hero/plans/pricing FAQ/signup all switched to "Start your free 30 day trial" (the pricing FAQ previously said there was NO free trial). *(Original spec, from uxpeak UX psychology analysis 2026-07-08: reciprocity — give the full product before asking for payment, the way Spotify gives 30 days of premium. Trial on REAL signup; the 7 day demo stays as is for browsers.)*
  - **Backend:** migration adds `tenants.trial_ends_at` + `tenants.trial_nudge_sent_at` (⚠️ single alembic head check before push). `POST /public/signup` gains a trial branch: create the tenant exactly as today (`is_demo=False`, chosen plan + modules recorded) but SKIP Stripe checkout creation and stamp `trial_ends_at = now + 30 days`. Conversion = existing Stripe checkout from `/settings/subscription` at any time; a completed checkout clears `trial_ends_at`. Expiry reuses the `subscription_expiry_check` pattern in `tickets/automation/sla_escalation.py`: new hourly `trial_expiry_check` sets `is_active=False` + sends prospect/admin emails. Nudge job emails at day 23 (reciprocity tone: what they've built so far) and day 28 (loss aversion tone: name their REAL contact/ticket counts). ⚠️ Interplay: the local uncommitted Stripe signup→checkout work (wired 2026-07-05) — the trial branch must fork BEFORE checkout creation.
  - **Frontend (apps/app):** persistent trial banner (same slot as the amber demo banner in `App.tsx`, shown when `trial_ends_at` is set): "Your free trial ends in N days" + Upgrade button → `/settings/subscription`; from day 25 the banner names concrete data ("Your 214 contacts and 38 tickets stay when you upgrade" — loss aversion, no em dashes in user copy). `TenantConfig` in `api/tenant.ts` gains `trial_ends_at`. SubscriptionPage shows trial status + the convert CTA.
  - **Marketing (apps/web):** hero + plans CTAs gain "Start your free 30 day trial" as primary (keep "Request demo" secondary); signup step 3 button becomes "Start 30 day free trial"; keep "No credit card required · Cancel any time".
...
- **[FLOW-CANVAS-DD] Drag-and-drop canvas (n8n-style)** — `Opus` — *after [FLOW4].* Free-position dragging + stored x/y in JSONB (no migration), user-drawn connections via React Flow handles + `onConnect`, node palette sidebar for drag-to-add, Backspace delete with undo toast. Modal stays for simple flows. Frontend only. Spec above.
- **[FLOW7] Flows: trigger registry + full trigger coverage** — `Opus` — per module `flow_triggers.py` declarations assembled into `TRIGGER_META`, drop the `TriggerType` Literal, `module` key + optgroup grouping in the picker, drift guard test, ~10 new triggers across livechat/calendar/marketing/contracts/billing/shipments/saas. Spec above. Do before [FLOW8]/[YIP-FLOW].
- **[FLOW8] Flows: built in automations — full inventarisation + /flows surface** — `Opus` — *after [FLOW7].* 16 automations catalogued (6 flows-expressible, retire hardcoded code; rest Phase A read-only cards). Key migrations: SLA escalation, Yip SLA nudge, booking→stage, ERP order→stage, campaign button click→stage/label all move into flows. Spec above with full table.
...
- **[FLOW6] Flows: flow chaining** — `Opus` — after phase 2; chain_depth ≤ 3 + cycle guard replaces the blanket `source="flow"` skip, opt in per flow.
...
- **[UX-CONV] Conversion psychology batch (uxpeak six)** — `Opus` — *companion to [UX-PSYCH] (retention); this one covers conversion/onboarding. From uxpeak's "The UX Psychology Behind Apps People Can't Stop Using" (2026-07-08), mapped against a full frontend inventory — only the verified gaps below; pricing page/signup step 2 already anchor well and were left alone.* Six sub items:
  1. **Smart defaults** (decision fatigue): `ContactNew` pre fills company when opened from a company context; `SendBookingModal` "propose" mode pre suggests the next 3 free slots from the availability grid; `YipTrainModal` gets per question skip with sensible defaults. Submit buttons state the outcome where they don't yet.
  2. **Goal gradient / endowed progress** (never start a user at 0%): `SetupChecklist` counts "Account created" as an always done first gate so it renders "1 of 5" and the bar never starts empty; `WelcomeTour` progress starts with step 1 filled.
  3. **Endowment in onboarding** (IKEA effect): add a "Make it yours" gate to `SetupChecklist` — pick brand colour + upload logo (reuse the branding PATCH `/team/branding` from session 46; verify the endpoint still exists before building).
  4. **Loss aversion surfaces:** `ModuleGate` stops silently redirecting to `/` — render the `PlanGate` style card naming the locked module + "Upgrade plan" CTA (superadmin contact for à la carte modules); the demo banner gets a concrete deadline ("Your demo ends Friday", from `demo_expires_at`) instead of "may be reset"; AI scan usage ≥90% shows an amber banner on SubscriptionPage + a once per session toast.
  5. **Anchoring** (contrast effect): homepage plan cards add the annual toggle + per user framing consistent with `/pricing` (reuse that page's logic); never show a flat price in isolation.
  6. **Dismiss copy audit:** upgrade/expiry prompts avoid soft "maybe later" outs; dismissals acknowledge the choice ("I'll risk it" pattern), phrased in Yippie's tone, no em dashes in user copy.
