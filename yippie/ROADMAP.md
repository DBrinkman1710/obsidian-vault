...
**[JARVIS1] Quick-capture assistant** — `Opus` — *post-launch.* A `⌘K` / `Ctrl+K` hotkey (configurable) opens a floating `QuickCaptureModal` over any page in the app. Single free-text input — the AI classifies intent and routes the action based on what the agent typed plus the current context (which contact, ticket, or page they're on). No page switching required.

**Three entry points:** (1) Global hotkey anywhere in the app. (2) Right-click on a contact row → "Add note". (3) Right-click on a ticket row → "Add note".

**Action types the AI routes to:**
- **Personal reminder** — "Remind me at 12:00 to follow up with Jan" → `user_reminders` row (`user_id`, `tenant_id`, `body`, `remind_at`); APScheduler minute-job fires a WebSocket in-app notification/toast at the scheduled time; dismissible.
- **Contact note** — when a contact is in context or named, appends a `quick_note` `activity_event` to that contact (visible in their timeline).
...
**Context injection:** the modal passes current page context (`context_type: "contact" | "ticket" | "none"`, `context_id`) to the backend so the AI skips disambiguation when context is obvious (e.g. typing "add note: interested in Growth plan" while viewing a contact immediately attaches it to that contact).

**DB:** `user_reminders` table (id, user_id, tenant_id, body, remind_at TIMESTAMPTZ, dismissed_at nullable, created_at); tenant-isolated via RLS. No new columns on existing tables — contact/ticket notes reuse `activity_events` with `event_type="quick_note"`.

**Frontend:** `QuickCaptureModal.tsx` — fixed center overlay, blurred backdrop, context chip showing the current contact/ticket name, type-ahead suggestions (recent contacts, open tickets), Enter to submit, Escape to close. Global `useQuickCapture()` hook (`⌘K`/`Ctrl+K` listener) registered in `App.tsx`. Right-click context menus on `ContactsPage.tsx` + `TicketList.tsx` rows pre-load the modal with the row's entity context.

**Backend:** `POST /quick-capture` receives `{body, context_type, context_id}`, calls Claude (Haiku for speed) to classify intent and extract entities (contact name, time expression, ticket ID), executes the action, returns `{action_taken, summary}` for a success toast. Anthropic API now; swap to [AI-MOD1] when available. Gated per tenant via `[AI-CTRL]`; counts against AI usage.

**The vision:** this is the "Jarvis" layer — a single always-available input that understands where you are and what you mean. Personal reminders and contact notes are phase 1. Future phases: natural-language queries ("how many open tickets do we have for Acme?"), task delegation ("create a follow-up ticket for Jan in 3 days"), and multi-step workflows — all from the same modal.
...
- **[ROI-FIX] ROI calculator broken — support staff variable not in equation** — `Haiku` — the ROI calculator on `getyippie.com` has a "support staff" slider but `staffCount` is not factored into the hours/€ saved formula. Fix so `staffCount` multiplies per-ticket savings. File: `apps/web/src/app/components/ROICalculator.tsx`. See Tier 3.
...
- **[PRIVACY-FIX] Privacy + terms page content errors** — `Haiku` — (1) Privacy page: `privacy@getyippie.com` → `support@getyippie.com`; (2) Terms page: remove "BV" from "Yippie BV" — not a BV yet, use "Yippie" or leave as "Yippie.io". Files: `apps/web/src/app/privacy/page.tsx`, `apps/web/src/app/terms/page.tsx`. See Tier 3.
...
- **[WEB-USE-NAV] Add "Use cases" to commercial site header nav** — `Haiku` — the current `SiteNav.tsx` does not have a "Use cases" link. Add it pointing to `/for-smbs` + `/for-agencies` (dropdown or separate link). See Tier 3.
- **[ABOUT-COMP] About page: remove company names, keep descriptions only** — `Haiku` — the `/about` page names specific companies. Replace with descriptive text only — no company names. File: `apps/web/src/app/about/page.tsx`. See Tier 3.
- **[BLOG-AUTHOR] Set blog post author to Diederik Brinkman** — `Haiku` — all blog posts show a placeholder author. Replace with "Diederik Brinkman" + a short bio blurb. Files: `apps/web/src/app/blog/…`. See Tier 3.
- **[VS-ZD-FIX] vs-zendesk page: remove AI inbox section, price from €19, no emojis** — `Haiku` — (1) Remove the "AI Inbox" comparison row/section; (2) Update starting price to "from €19/month"; (3) Strip all emojis from the page. File: `apps/web/src/app/vs-zendesk/page.tsx`. See Tier 3.
...
- **[JARVIS1] Quick-capture assistant** — `Opus` — *post-launch.* Global `⌘K` hotkey opens a floating modal anywhere in the app. Type anything; AI classifies intent (personal reminder, contact note, ticket note, navigation) and routes to the right action based on current page context. Right-click "Add note" on contact/ticket rows as a shortcut. `user_reminders` table + APScheduler for timed reminders; contact/ticket notes go into `activity_events`. Uses Anthropic API (Haiku) early; upgrades to [AI-MOD1]. The Jarvis foundation — phase 1 is notes/reminders; future phases add natural-language queries and multi-step workflows. See post-launch build order + spec above.
...
- **[LIVECHAT-1PC] WhatsApp livechat: one session per contact** — `Sonnet` — *not built.* Agents end up with multiple open sessions for the same WhatsApp number. Fix: in `chat/router.py` `receive_message` and the agent-initiated create path, always query for an existing open session by `(tenant_id, whatsapp_phone)` before creating; reuse if found. Covers both inbound webhook and `POST /chat/sessions`. Files: `chat/router.py`, `chat/service.py`.
...
- **[MKTG-PAGE] Marketing module in-app page upgrade** — `Sonnet` — *not built.* Current marketing page is primarily a template editor. Build a full campaign dashboard as the module landing page: campaign list (status, sent/open%/reply% stats), audience segments summary, and drip sequence overview. Template editor accessible per-campaign via "Design" tab. The spec is in [MKTG1] (session 69); this is the front-end depth pass.
...
- **[BK-DATE] Booking: available dates cut off around 3 July** — `Haiku` — *not built.* Investigate `expiry_days` in `calendar_settings` + `get_available_slots()` lookahead cap. Raise default lookahead to ≥60 days or make it configurable. Files: `booking/service.py`, `BookingSettingsModal.tsx`.
- **[DEMO-BUG] Request demo: "Something went wrong" error** — `Haiku` — *not built.* Check `apps/web/src/app/api/request-demo/route.ts` proxy and backend `public/router.py` for schema mismatch after [BIZ-Q1] refactor (questionnaire field now required?), CORS, or Railway env issues.
- **[EMAILTRACK-NAV] Email tracking sidebar link opens billing page** — `Haiku` — *not built.* Check `MODULE_MAP` in `Sidebar.tsx` and route in `App.tsx` for wrong path on the emailtracking/marketing module entry.
- **[ROI-FIX] ROI calculator: support staff variable missing from equation** — `Haiku` — *not built.* `staffCount` slider is displayed but not used in the formula. Fix so it multiplies per-ticket savings. File: `apps/web/src/app/components/ROICalculator.tsx`.
- **[BOOK-LOGO] Booking page shows old logo** — `Haiku` — *not built.* Fix logo in `BookingPage.tsx` and `MeetPage.tsx` to use current asset or tenant `BRANDING_LOGO_URL`.
- **[PRIVACY-FIX] Privacy + terms page content errors** — `Haiku` — *not built.* (1) Privacy page: `privacy@getyippie.com` → `support@getyippie.com`; (2) Terms page: remove "BV" from "Yippie BV". Files: `apps/web/src/app/privacy/page.tsx`, `apps/web/src/app/terms/page.tsx`.

### Commercial site polish (2026-06-22)
- **[WEB-CENTER] Commercial site pages not centred consistently** — `Haiku` — *not built.* Audit all pages (home, pricing, modules, vs-zendesk, blog, about, privacy, terms, request-demo) for consistent `max-width: 1180px` centering. Apply the standard `.navInner` centering pattern throughout.
- **[WEB-USE-NAV] Add "Use cases" to commercial site header nav** — `Haiku` — *not built.* Add "Use cases" link/dropdown to `SiteNav.tsx` pointing to `/for-smbs` and `/for-agencies`. File: `apps/web/src/app/components/SiteChrome.tsx` / `SiteNav.tsx`.
- **[ABOUT-COMP] About page: remove company names, keep descriptions** — `Haiku` — *not built.* Replace named companies with descriptive text only. File: `apps/web/src/app/about/page.tsx`.
- **[BLOG-AUTHOR] Set blog author to Diederik Brinkman** — `Haiku` — *not built.* Replace placeholder author with "Diederik Brinkman" + short bio across all blog posts. Files: `apps/web/src/app/blog/…`.
- **[VS-ZD-FIX] vs-zendesk page fixes** — `Haiku` — *not built.* (1) Remove AI inbox comparison section; (2) Starting price → "from €19/month"; (3) Strip all emojis. File: `apps/web/src/app/vs-zendesk/page.tsx`.
- **[FOUNDER-PLAN] Founder plan: 10 users + 50% module discount** — `Haiku` — *not built.* Update `plans.py` `PLAN_LIMITS` + Founder card on getyippie.com pricing page: max 10 users, 50% off all paid add-on modules.
---
